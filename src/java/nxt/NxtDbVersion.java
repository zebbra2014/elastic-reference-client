        ...
        switch (nextUpdate) {
            ...
            case 17:
            	apply("CREATE TABLE IF NOT EXISTS work (db_id IDENTITY, id BIGINT NOT NULL, work_title VARCHAR NOT NULL, variables_input SMALLINT NOT NULL, variables_output SMALLINT NOT NULL, version_id SMALLINT NOT NULL, language_id SMALLINT NOT NULL, "
                        + "deadline INT NOT NULL, num_bounties INT NOT NULL, num_pow INT NOT NULL, original_amount BIGINT NOT NULL, paid_amount_bounties BIGINT NOT NULL, paid_amount_pow BIGINT NOT NULL, fee BIGINT NOT NULL, referenced_transaction_id BIGINT, FOREIGN KEY (referenced_transaction_id) REFERENCES transaction (id) ON DELETE CASCADE, block_id BIGINT NOT NULL, FOREIGN KEY (block_id) REFERENCES block (id) ON DELETE CASCADE, "
                        + "sender_account_id BIGINT NOT NULL, code OTHER NOT NULL, payback_transaction_id BIGINT, FOREIGN KEY (payback_transaction_id) REFERENCES transaction (id) ON DELETE CASCADE, last_payment_transaction_id BIGINT, FOREIGN KEY (last_payment_transaction_id) REFERENCES transaction (id) ON DELETE CASCADE)");
            case 18:
            ...
            case 193:
                return;
            default:
                throw new RuntimeException("Blockchain database inconsistent with code, probably trying to run older code on newer database. Requested update version was: " + String.valueOf(nextUpdate));
        }
    }
