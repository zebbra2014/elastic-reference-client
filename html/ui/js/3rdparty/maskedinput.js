...
! function($) {
			...
			return !mask && this.length > 0 ? (input = $(this[0]), input.data($.mask.dataName)()) : (settings = $.extend({
				...
				firstNonMaskPos = null, $.each(mask.split(""), function(i, c) {
					...
					function keypressEvent(e) {
						...
						if (/^XEL\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(input.val())) {
					...
					function checkVal(allow) {
						input.val(input.val().replace(/^\s*XEL\-\s*XEL/i, "XEL-"));
					...
					if (settings.noMask) {
						input.bind("keyup.remask", function(e) {
							...
							setTimeout(function() {
								...
								if (/^XEL\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(newInput) || /^XEL[A-Z0-9]{17}/i.test(newInput)) {
					...
					input.data($.mask.dataName, function() {
						...
						setTimeout(function() {
							...
							if (/^XEL\-[0-9]{19,20}$/i.test(pasted)) {
								//old style accounts..
								input.val("").trigger("oldRecipientPaste");
							} else {
								var match = /^XEL\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.exec(pasted);

								if (match && match[0]) {
									input.val(match[0]).trigger("checkRecipient");
								} else {
									match = /^XEL[A-Z0-9]{17}/i.exec(pasted);
									if (match && match[0]) {
	...
	function text_diff(first, second) {
		...
		if (/^XEL\-/i.test(second) && !/^XEL\-/i.test(diff)) {
			diff = "XEL-" + diff;
		}
